#!/bin/bash
# Proxmox PaaS Platform - Complete Setup Script
# This script sets up everything on Proxmox server

set -e

echo "=========================================="
echo "Proxmox PaaS Platform - Setup"
echo "=========================================="
echo ""

# Configuration
PROXMOX_IP="192.168.171.140"
INSTALL_DIR="/root/proxmox-paas"
API_PORT="5000"
...
echo ""
echo "📥 Step 5: Downloading Ubuntu LXC template..."
...
echo ""
echo "📝 Step 6: Creating PaaS API application..."
...
FRAMEWORKS = {
    ...
    'python-django': {
        ...
        'run_cmd': '/opt/app/venv/bin/gunicorn --bind 0.0.0.0:8000 --workers 3 $(find . -name wsgi.py | head -1 | sed "s|./||" | sed "s|/|.|g" | sed "s|.py||"):application',
    },
    'python-fastapi': {
        'name': 'Python FastAPI',
        'port': 8000,
    ...
    'php-laravel': {
        'name': 'PHP Laravel',
        'port': 8000,
        'install': 'apt-get update && apt-get install -y php php-cli php-mysql php-xml php-mbstring php-curl php-zip unzip git curl && curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer',
    ...
    'rust-actix': {
        'name': 'Rust Actix',
        'port': 8080,
        'install': 'apt-get update && apt-get install -y curl build-essential git && curl --proto "=https" --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y',
...
def deploy():
    ...
    if framework not in FRAMEWORKS:
        return jsonify({'error': f'Unknown framework'}), 400
...
echo ""
echo "🎨 Step 7: Creating web interface template..."
# The templates/index.html file should be uploaded separately or created
...
ExecStart=$INSTALL_DIR/venv/bin/gunicorn --bind 0.0.0.0:$API_PORT --workers 4 --timeout 300 app:app
Restart=always
RestartSec=10
...
echo "=========================================="
echo ""
echo "PaaS Platform is running at:"
echo "  🌐 http://$PROXMOX_IP:$API_PORT"
echo ""
echo "Service Management:"
echo "  Status:  systemctl status proxmox-paas"
echo "  Restart: systemctl restart proxmox-paas"
echo "  Logs:    journalctl -u proxmox-paas -f"
echo ""
...
echo "  🦀 Rust Actix"
echo "  💎 Ruby on Rails"
echo "  🌐 Static Sites (Nginx)"
echo ""
echo "Next Steps:"
echo "  1. Upload templates/index.html to $INSTALL_DIR/templates/"
